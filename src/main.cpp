...
int OnePeriod; // меняется каждую секунду
...
template<typename T>
void prin_2_serial(const T& mystr) {
  Serial.print(millis());
  Serial.print(" ");
...
template<typename T>
bool print_on_LCD(int numLCD, int X, int Y, bool bClear, const T& mystr) {
  prin_2_serial("Begin.print_on_LCD");
  Blynk.run();
  if (Blynk.connect(SERVER_TIMEOUT)) {
    ...
    if (numLCD  == 1) {
      ...
      //Serial.print("lcd1.print(X, Y, mystr)",X,Y,mystr);
    } else if (numLCD == 2) {
      lcd2.print(X, Y, mystr);
    }
    prin_2_serial("End.true.print_on_LCD");
    return true;
  } else {
    prin_2_serial("End.false.print_on_LCD");
    return false;
  }
}
...
void send_2_blynk(int numLCD) {
  prin_2_serial("Begin.send_2_blynk");
  Blynk.run();
  if (Blynk.connect(SERVER_TIMEOUT)) {
    if (numLCD  == 1) {
      lcd1.clear();
    ...
    prin_2_serial("Blynk.connect error");
  }
  prin_2_serial("End.send_2_blynk");
}
...
void setup() {
  ...
  Serial.begin(115200);
  prin_2_serial("Booting");  //  "Загрузка"
  // WIFI_OFF = 0,
  // WIFI_STA = 1 - клиент,
  ...
  while (WiFi.waitForConnectResult() != WL_CONNECTED) {
    prin_2_serial("Connection Failed! Rebooting...");
  ...
  prin_2_serial("IP address: ");  //  "IP-адрес: "
  prin_2_serial(WiFi.localIP());
